...
type OrderTrackingServiceInterface interface {
	GetOrderTracking(ctx context.Context, req model.OrderTrackingRequest) (res interface{}, err error)
}

func (s *OrderTrackingService) GetOrderTracking(ctx context.Context, req model.OrderTrackingRequest) (res interface{}, err error) {
	...
	if len(lstOrderTracking.Data) > 0 {
		...
		if err != nil {
			...
			return nil, ginext.NewError(http.StatusBadRequest, "Error when get order")
		}
